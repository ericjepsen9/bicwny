...
## 第 2 层 · 首页卡仲裁（单槽位竞争）· ❌ 已废弃 · 不实施

> # 🚫 此设计已作废 · 不实施 · 不要据此写代码
>
> **作废原因**：在第 8 层做出 B 方案简化决策 · 改用「液态玻璃文字 + 班级红点 + In-app Banner」三层组合替代「单槽位首页卡 + 优先级仲裁」。
>
> **替代方案**：见第 8 层 · 或 `NOTIFICATION_FINAL_SPEC.md` §6 §7（最终设计）。
>
> **以下原文保留仅作历史参考** · 不应被视为待实施任务：
> - 优先级表（14 行评分系统）❌
> - 4 档窗口（预告 / 临近 / 倒数 / 进行中）❌
> - HomeCardDismissal 表 ❌
> - 单槽位仲裁逻辑 ❌
> - contentHash dismissal 失效（这个保留 · 但仅用于 critical 系统公告 ack）
>
> 请直接跳到第 8 层阅读 · 或参考 FINAL_SPEC。
...
## 第 6 层 · 首页卡 UI 细节 · ❌ 已废弃 · 不实施

> # 🚫 此设计已作废 · 不实施 · 不要据此写代码
>
> **作废原因**：第 8 层 B 方案不再使用「首页卡」概念 · 改为「玻璃文字 + Banner」组合。
>
> **替代方案**：
> - 临场提醒（共修 T-5 / T-0 等）→ In-app Banner（spec §6 · v3 实施）
> - 静态概览（下次共修 / 今日任务 / 最新公告）→ 首页玻璃文字（spec §7 · v3 实施）
> - 单场详情 → 班级共修详情页 `/class/:id/sessions/:sid`（已实施）
>
> **以下原文保留仅作历史参考** · 不应被视为待实施任务：
> - 9 类卡形草图 ❌（Banner / 详情页接管）
> - 倒计时三阶段配色（已在班级共修详情页实现 · 非「卡片」形式）
> - 档位切换动画（已在详情页实现）
> - dismiss / ack 交互（仅 critical 系统公告 ack 保留 · v3 Banner 实施）
>
> 请直接跳到第 8 层阅读 · 或参考 FINAL_SPEC §6 §7。
