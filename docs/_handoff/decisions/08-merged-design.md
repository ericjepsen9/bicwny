...
>
> **终态约束实现方式（DR-182）**：`advanced` 和 `disqualified` 的终态保护**由应用层保证，无 DB trigger**（用户决策 2026-06-05）。实现注意：每个写 `cohortStatus` 的端点（留级、退出、升学、取消资格、复活）均须在写入前校验当前状态是否为终态，若是则拒绝操作并返回 4xx；禁止在任何端点中绕过此校验直接写库。
