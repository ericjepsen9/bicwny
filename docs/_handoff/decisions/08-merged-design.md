...
>
> **⚠️ advanced → graduated 受限例外（DR-184）**：仅 super_admin 可执行「撤销升学」操作（误操作纠正），执行后 cohortStatus 从 advanced 回到 graduated；必须填写撤销原因（必填文字字段），并写 AuditLog（actionType=advancement_decision, payload={result:'revoked', operatorId, reason, revokedAt}）；AdvancementRecord 原行保留不删（D18），AuditLog 提供时序凭证；撤销后学员回到 graduated，管理员可重新走升学审核流程。**此路径不对 class_admin 及以下开放。**
>
...
| DR-184 | 缺口闭合：advanced → graduated 受限例外——super_admin 撤销误操作升学路径（2026-06-05）| **受限例外，仅 super_admin**：`advanced` 原则上是永久终态（DR-150/DR-178），但新增一条受限例外路径：super_admin 可执行「撤销升学」操作，将 cohortStatus 从 `advanced` 回写至 `graduated`（误操作纠正）。**执行条件**：① 操作者角色 = super_admin；② 必填撤销原因（文字字段）；③ 同步写 AuditLog(actionType=advancement_decision, payload={result:'revoked', operatorId, reason, revokedAt})。**AdvancementRecord 处理**：原行保留不删（D18），AuditLog 作为撤销的时序凭证；档案视图显示「曾升学通过 → 后被撤销」完整历史。**撤销后**：cohortStatus=graduated，管理员可重新走升学审核（graduated→advanced 或 graduated→held_back），无需重跑预检。**实现要求**：「撤销升学」须作为独立端点（super_admin 守卫），其余所有写 cohortStatus 的端点校验逻辑不变——advanced 仍对 class_admin 及以下是终态（用户决策 2026-06-05）| 排除「接受风险，不做撤销（A）」：advanced 终态不可撤销，误操作后只能走技术人员 DB 修正，缺乏治理路径，违反 D17 高权限操作留痕精神；排除「AdvancementRecord 加 isRevoked 字段（B）」：新增字段改变表结构，且 AuditLog 已能承载撤销证明（payload 自由 Json），无需为此扩表；排除「对 class_admin 开放撤销」：升学审核本身是 class_admin+ 操作，撤销权限须高于批准权限（super_admin）才能防止自我撤销；排除「advanced→active 直接复活」：advanced 是原专业终态，回 graduated 是最接近误操作前状态的语义最纯路径（可重走审核）。关联 DR-150（advanced 终态）/ DR-178（转移矩阵）/ DR-182（终态应用层保护）/ DR-174（AdvancementRecord 只写 passed）/ §3.11 AuditLog / D17 / D18 / 能力 10（升学审核）/ 能力 11 |
...
| DR-180 | T4 闭合：留级后进度页「本届说明」横幅——触发条件 ClassMember.heldBackCount > 0，文案说明三类口径差别（2026-06-05）| **条件展示横幅**：学员进度页，当前 `ClassMember.heldBackCount > 0` 时（即留级过的学员），页面顶部显示说明横幅：「本届起算：92修法 · 共修出勤从加入本班起计。心咒累计（顶礼/心咒打卡）保留全量历史。」横幅可手动关闭，前端以 `localStorage` 按 `classId+userId` 记忆关闭状态，每届班级只提示一次。**触发条件**：`heldBackCount > 0` 是最直接信号，已在 ClassMember 现有 API 响应字段中，无需新端点。**转专业学员（heldBackCount=0）**：⏸ 暂缓——转专业场景的管理员侧已由 DR-179 确认弹窗覆盖，学员侧横幅触发逻辑待后续优化。无新表/无新字段/无新 API/无新 migration，纯前端逻辑（用户决策 2026-06-05）| 排除「不做UI提示（A）」：三类口径差别是留级学员高频困惑点，管理员侧已做退出说明（DR-179），学员侧也应知情，依赖辅导员口头告知不稳定；排除「所有学员永久显示小注（C）」：对首次入班学员"本届起算"信息无意义，增加认知负担；选 B（条件展示）在正确时机面向正确对象，信噪比最优。排除「heldBackCount=0 但有历史打卡时也触发」：转专业场景由 DR-179 覆盖，进度页再加另一套触发逻辑复杂度高，⏸ 留后续优化。关联 DR-170/DR-157（cumulative_count 全量历史）/ DR-161（practice_session 起修日过滤）/ DR-163（attendance current_member）/ DR-179（转专业管理员侧提示）/ 能力 11 / R3 剧本 T4 |
