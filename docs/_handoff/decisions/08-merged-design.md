...
**判定逻辑**：`GROUP BY meditationId WHERE userId=:id AND loggedAt >= :programStartDate`（`:programStartDate` = `Program.startDate` 起修日，ISG-4 闭合，DR-161）→ 每组 `COUNT ≥ minSessionsPerItem AND SUM(durationMinutes) ≥ minMinutesPerItem` → 满足组数 = `itemCount` AND 全局 `COUNT ≥ totalMinSessions` AND `SUM ≥ totalMinMinutes`。双维度独立达标（DR-91）。起修日前（S1）的修量不计入升学预检（能力 4 规则 8）。
...
| 2026-06-04 | DR-161 ISG-4 闭合——`practice_session` 判定逻辑补 `loggedAt >= Program.startDate`（起修日）过滤；S1 阶段修量不自动计入升学预检（能力 4 规则 8）；管理员通过能力 5 代行补录的 S1 修量若需计入升学须以 `loggedAt >= startDate` 记录（行政行为非系统自动）；§3.1 practice_session 判定逻辑行内更新；无新表/无新字段/无新 migration |
...
| DR-161 | ISG-4 闭合：`practice_session` conditionType 判定逻辑补起修日过滤（2026-06-04）| **`loggedAt >= Program.startDate`**：预检聚合范围限定为起修日（`Program.startDate`，即 S2 开始时间/UserPracticeVow 建立时间）之后的 PracticeLog；起修日前（S1）的修量不自动计入升学聚合（能力 4 规则 8）。管理员通过能力 5 代行补录 S1 修量若需计入升学，须将 `loggedAt` 设为 >= startDate（行政行为，非系统自动）（用户决策 2026-06-04）| 排除「不加时间过滤」：S1 阶段通过 source=external 申报或管理员补录的座数会被预检计入，违反"起修日之前不算"硬规则（能力 4 规则 8）；排除「用 vowId 隐式过滤（vowId 创建于起修日）」：vowId 是必填字段（DR-153），可作为辅助保证，但判定逻辑中明文 loggedAt 时间过滤更直接、可读性更高、不依赖 vow 实现细节。关联 能力 4 规则 8 / G1.3（S1 修量代行补录方案 B）/ DR-98（practice_session params）/ ISG-4 |
