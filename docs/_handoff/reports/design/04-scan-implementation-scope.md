...
|---|---|---|---|---|---|---|---|
| **ISG-1** | 08 §3.1 ProgramAdvancementConfig·conditionType 表·`course_completion` 行；08 §3.1「各 conditionType 标准 params 结构」节 | "全部正式课程闻思圆满 \| 能力 3"；`params` 结构节**只列了 `practice_session`/`exam_score`/`cumulative_count`，无 `course_completion` 的 params 结构**，无判定逻辑说明 | **① 课程过滤字段：** 判定哪些课程属于"本专业的全部正式课程"——靠 `Course.programSemesterId`？靠 `ProgramAdvancementConfig.params` 内的 courseIds 列表？还是查 programId 关联的全部 `courseType=entry/formal` 的课程？ **② 跨班级共享课程的处理：** 同一门法本被多个专业引用时，闻思圆满算"该学员在该专业的完成记录"还是"全局完成记录"？ | false-negative | 若过滤范围太窄（如只查某班的课程），学员明明把专业所有正式课都学完但预检仍报"未达标" | 补一节 `course_completion` 标准 params 结构（对齐 DR-97 格式），明文：聚合用 `WHERE userId=:uid AND lessonId IN (本专业 entry/formal 课程的全部 lessonId)` 得出的 LessonCompletion 记录，并说明跨班共享课程的口径（建议：只要 userId 的圆满态为 true 即认可，不限 classId） | 无既有 GAP 同源；是 DR-97 params 完整性遗漏（TODO-17 仅覆盖了其余 3 种 conditionType） |
...
| **ISG-9** | 06 能力 3 规则 7「多专业并行下课程独立：不共享、不豁免」；08 §3.9 AdvancementCheck；08 §3.1 course_completion | 06 能力 3：「多专业并行下课程独立：不共享、不豁免（各自完成）」；但**当同一门法本（Course）被多个专业引用时，学员在 A 专业完成该课的 LessonCompletion 记录，在 B 专业的 course_completion 预检中能否计入**，文档无明文决策 | **跨专业课程圆满能否复用：** 同一 lessonId + userId 的 LessonCompletion 记录是"全局单条"还是"按专业隔离"？LessonCompletion 表（08 §三 DR-129）无 programId 字段，无法按专业过滤。如果预检按"全局查 LessonCompletion"，则 A 专业的学习记录会使 B 专业的 course_completion 条件也满足——与 06 能力 3 规则 7 明文冲突 | false-positive | A 专业学完某法本 → 该法本的 LessonCompletion 存在 → B 专业预检也认为此法本"已圆满" → B 专业 course_completion 条件被 A 专业学习记录满足，违反"课程各自完成"原则 | 在 course_completion 的判定逻辑中明文：仅计入"学员以本专业学员身份应完成的课程"——需在 LessonCompletion 中加 `programId` 字段（类似 PracticeLog.programId），或 course_completion 预检逻辑中只计入 LessonId IN（本专业课程集合）且不跨专业共享；若决策是"闻思可跨专业共享"则需在 06 能力 3 规则 7 修订此例外 | 与 D14b 日常不豁免的逻辑冲突风险类似；LessonCompletion 缺 programId 字段是该问题的数据根因 |
...
- 其中实现范围空白（本报告新发现）：**12 条**（ISG-1 至 ISG-12）
