...
#### S-004 首页展示（有平台活动） ⚠️ 设计有缺口（布局待定）

**触发**：学员打开 App，进入 `/`（HomePage）。

**HomePage**
- 🖥️ 看到：
  - 顶部：左侧头像（挂未读通知红点 badge）+ 右侧药丸卡片
  - 班级信息卡 / 进度信息
  - 下方各功能入口
- 👆 可以做：点击头像 → ProfilePage；点击药丸卡片 → `/events` 活动中心
- ➡️ 药丸卡片行为（数据来源：`GET /api/activities/summary`）：
...
- ⚠️ 边缘情况：
  - `⚠️ 待决策`：是否实现独立 `GET /api/activities/summary` 或复用 `GET /api/my/upcoming-events?within=10080`
  - 通知入口已移入「我的」页面，头像 badge 为补偿；现有 ProfilePage 已有通知入口

---

#### S-005 首页展示（无平台活动） ⚠️ 设计有缺口

**触发**：平台无任何进行中 / 即将开始活动。

**HomePage 药丸卡片**
- 🖥️ 看到：`📅 平台活动` + 日历图标，无角标
- 👆 可以做：点击 → 跳 `/events`，进入往期列表
- ➡️ 之后：活动中心页各 Tab 均显示「进行中」空态 + 往期记录列表
...
- 🖥️ 看到：
  ...
  - **区块①「班级修学愿」**（source=auto）：入班时按 PracticeTemplate 自动建
    - 每条：愿名 + 进度条（按 targetPeriod 主目标）+ 「去打卡」按钮
    - 进度条：`currentCount / targetCount`（lifetime）或今日 / 本周打卡量
  - **区块②「我的修学」**（source=custom）：用户自建
    - isPledged=true（发愿）：有进度条 + 目标量
    - isPledged=false（裸追踪项）：仅累计数 + `+` 快捷打卡
    - `+ 添加修学` 按钮
- 👆 可以做：
  - 点「去打卡」→ 触发打卡流程（S-007）
  - 点「+ 添加修学」→ 触发创建流程（S-009）
  - 点裸追踪项 `+` → 直接进入打卡
  - 点愿的进度条 → 展开愿详情（含历史打卡记录）
- ⚠️ 边缘情况：
...
#### S-007 修持打卡（发愿 / 裸追踪） 🔵 已设计未实现

**触发**：在 PracticePage 点击任意愿的「去打卡」。
...
- ⚠️ 边缘情况：
  ...
  - 补签：`source='makeup'`，`logDate` 指向目标历史日期（非 now）；同周补签配额 1 次（`Serializable` 事务内检查）
  - preferShowFaxin=false：跳过发心语和回向，直接提交后返回

---

#### S-008 查看打卡历史 ✅ 已实现（基础）/ 🔵 新字段未实现

**触发**：在 PracticePage 点击愿条目或底部「历史」入口。

**PracticeHistoryPage**
- 🖥️ 看到：按日期倒序的打卡记录列表，含修法项目名 / 遍数 / 时长 / 座次 / 来源（manual/makeup 等）
- 👆 可以做：按日期/项目筛选
- ⚠️ 边缘情况：`PracticeEntry` 旧表已删除，历史打卡需已迁移至 `PracticeLog`
...
- ⚠️ 边缘情况：裸打卡不可补发愿（有意设计）；要发愿须新建 isPledged=true 的愿，历史裸打卡不追溯
...
#### S-012 法会活动中心（法会 Tab） 🔵 已设计未实现

**触发**：点击首页药丸卡片 → `/events`，选择「法会」Tab。

**EventsPage 法会 Tab**
- 🖥️ 看到（三分区）：
  - **正在进行**：封面图 + 标题 + 藏历日期 + 「还剩 N 天」倒计时 + 橙色「参与」按钮
  - **即将开始**：同上，按钮改为「预发愿」
  - **往期法会**（折叠）：展开后显示标题 + 日期区间 + 参与人数
- 👆 可以做：点任意法会卡片 → `/events/:id` 法会详情页
- ⚠️ 边缘情况：往期法会默认折叠，防止页面过长
...
**区块 2：共修总量（实时）**
- 🖥️ 看到：按 practiceProjectId 分组：`上师瑜伽 · 共 12,450 遍 · 38 人参与`
- 进行中时：30 秒轮询刷新 + 新增时动效
- 已结束时：静态最终总量（供回向参考）
...
| 法会未开始，有愿 | 愿进度条（0 / 目标量）| 发愿按钮（调整愿）|
| 进行中，未提交 | 两个并排按钮 | `发法会愿` / `去打卡` |
| 进行中，有愿 | 愿进度条（已完成 / 目标量）+ 按钮 | `去打卡`（触发 S-015）|
| 进行中，无愿 | 已提交 N 次，合计 X 遍 | `继续打卡` |
| 已结束，有提交 | 我的最终总量：X 遍 | `回向`（仅结束后出现，S-016）|
| 已结束，无提交 | 「此法会已结束」 | 无操作按钮 |

- ⚠️ 边缘情况：
  - 即将开始时「去打卡」按钮不可用（tooltip 提示），「发法会愿」正常可用
...
**发愿 Sheet**
- 🖥️ 看到：修法项目选择 + 目标量输入（lifetime 总量）+ 起始日只读（= event.startDate 或 today，取较大值）
- 👆 可以做：选项目，填目标量，提交
- ➡️ 之后：`POST /api/events/:id/vow` 写 `UserPracticeVow{ context='event', eventId, source='custom', startDate=max(event.startDate, today) }`，区块 3 切换到「有愿」状态
- ⚠️ 边缘情况：同一事件重复发愿 → 后端应检查已有愿，弹提示或支持修改；today 按 `event.timezone` 计算

---

#### S-015 法会打卡（提交计数） 🔵 已设计未实现

**触发**：法会详情页点「去打卡」/「继续打卡」。

**计数提交 Sheet**
- 🖥️ 看到：修法项目选择（有愿时预填）+ 遍数输入（整数，必填；法会计数不记时长/座次）
- 👆 可以做：填遍数，提交
- ➡️ 之后：
  ...
  3. 区块 2 共修总量实时 +N 动效
  4. Sheet 关闭，**不弹回向 Sheet**（法会打卡不触发三殊胜回向）
- ⚠️ 边缘情况：
...
**触发**：法会已结束，区块 3 有提交记录，点「回向」按钮。

**回向 Sheet**
- 🖥️ 看到：此次法会共修总量 + 固定回向文字（前端常量）+ 「完成回向」按钮
- 👆 可以做：点「完成回向」
- ➡️ 之后：Sheet 关闭，**不写 DB**（纯 UI 仪式）
...
#### S-017 活动中心共修 Tab 🔵 已设计未实现

**触发**：EventsPage 切换到「共修」Tab。

**EventsPage 共修 Tab**
- 🖥️ 看到（三分区）：
  - **进行中**（`startAt ≤ now ≤ sessionEndAt`）：标题 + 课时 + 时间窗口 + 「去签到」按钮（App 内）
  - **即将开始**（`startAt > now`）：标题 + 课时 + 开始时间 + 「设提醒」
  - **往期**（`sessionEndAt < now`，折叠）：标题 + 日期 + 我的出勤状态
- 👆 可以做：点「去签到」→ App 内签到（S-019）/ 或点链接签到（S-025）
- ⚠️ 边缘情况：仅展示 classId=null 的平台级共修；班级级共修仍在班级页「共修安排」卡
...
#### S-020 讲考 Tab + 报名 🔵 已设计未实现

**触发**：EventsPage 切换到「讲考」Tab。
...
**触发**：点底部导航「日历」→ `/calendar`（CalendarPage）。
...
| 编号 | 位置 | 缺口描述 |
|---|---|---|
| G-001 | S-004 药丸卡片 | `⚠️ 待决策`：数据源是独立 `/api/activities/summary` 还是复用现有 `/api/my/upcoming-events` |
| G-002 | S-004 药丸卡片 | 布局（「活动按钮 + 通知铃」→ 药丸）待最终确认，当前方案「暂定」 |
| G-003 | S-014 发法会愿 | 同一事件重复发愿的处理逻辑未明确（返回旧愿 or 报错 or 允许多愿）|
| G-004 | S-022 讲考签到 | `speaking_present` 的写法需与 studyType 枚举值对齐（设计文档写 `speaking_pass` 和 `speaking_present` 两处不一致）|
...
| G-008 | 全局 | `StudyRecord.studyType='speaking_pass'`（设计文档 §3.1 CheckIn 模块）与枚举定义 `speaking_present`（§2.3 StudyRecord 表定义）不一致，需统一 |
