...
**~~区块 2：共修总量~~**（❌ 已移除，2026-05-27）
- 法会计数改为线下上报，App 不再追踪个人提交量，此区块删除。

**区块 2（新）：发愿人数**
- 🖥️ 看到：`🙏 已有 N 人发愿`（从 UserPracticeVow 计数，context=event AND eventId=:id）
- 静态展示，不轮询
...
| 用户状态 | 展示 | 操作 |
|---|---|---|
| 法会任何状态，无愿 | `发法会愿` 按钮 | 触发发愿 Sheet（S-014）|
| 法会未结束，有愿 | 我的发愿：N 遍 | `回向` |
| 法会已结束，有愿 | 我的发愿：N 遍 | `回向` |
| 法会已结束，无愿 | 「此法会已结束」 | 无操作 |

- ⚠️ 边缘情况：
  - 有愿即可触发回向，不依赖提交记录（发愿数量 = 承诺量，线下自行完成）
...
#### S-015 法会记数 ❌ 已废弃（2026-05-27）

法会计数改为线下上报，App 不再提供个人记数入口。
`POST /api/events/:id/count` 从学员端路由移除；`EventCount` 表保留历史数据，不再写入。

---

#### S-016 法会回向仪式 🔵 已设计未实现

**触发**：区块 3 **有法会愿**即可点「回向」（法会未开始/进行中/已结束均可）。
...
| 学员 | 出勤 | 闻思内容 | 答题 | 观修 | 修持任务 |
|---|---|---|---|---|---|
| 张三 | ✅ on_track | 🔴 at_risk | 🟡 slightly_behind | ✅ on_track | 🟠 falling_behind |

- 五维度说明：
  - **出勤**：近2周必修场次签到率（无排表必修场次 → 恒 on_track）
  - **闻思内容**：近2周法本阅读/音频/视频标记「已学习」完成率（无排表 → 恒 on_track）
  - **答题**：近2周完成排表课时关联题目的比例（无排表 → 恒 on_track）
  - **观修**：近2周观修标记「已完成」比例（无排表 → 恒 on_track）
  - **修持任务**：近2周有打卡记录的天数 / `lagPracticeDaysExpected`（默认 10 天，admin 可按班调）
- 👆 可以做：
  - 按任一维度筛选/排序（如按观修维度 at_risk 排序）
  - 点「detail」→ 查看该学员明细数据（各维度 done / expected 数）
