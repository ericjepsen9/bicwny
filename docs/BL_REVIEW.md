...
**白话**：学员在登录页点「忘记密码」，输入邮箱，收到邮件后点链接设新密码；密码重置后所有设备强制下线。

**流程一：申请重置（`/forgot`）**
1. 输入邮箱 → `POST /api/auth/forgot`
2. 后端生成 256 位随机 token，DB 存 SHA-256 哈希，30 分钟有效
3. 无论邮箱是否存在，响应**一律** `{ ok: true }`（防邮箱枚举攻击）
4. 页面切换到成功态：「请检查邮箱（可能在垃圾箱）· 链接 30 分钟内有效」

**流程二：设置新密码（`/reset?token=…`）**
1. 邮件链接携带 token，前端读 URL 参数
2. 输入新密码（≥ 6 位）+ 确认密码 → `POST /api/auth/reset`
3. 后端原子事务：更新密码哈希 + 标记 token 已用 + 作废同用户其他未用 token + 吊销所有活跃 session
4. 成功后跳转 `/auth` 重新登录

**安全规则：**
- 未验证邮箱 / 已停用账户 / 无密码账户 → 静默不发邮件（防信息泄漏）
- 单用户 1 小时内最多申请 3 次（防 DOS 轰炸受害邮箱）
- 每次申请自动作废该用户所有旧未用 token（防历史邮件截获）
- token 不存在 / 已使用 / 已过期 → 统一返回「token 无效或已过期」（防差异探测）

**⚠️ 待接入**：生产环境 SMTP / SendGrid 尚未配置，当前 dev 模式通过日志输出链接调试。
...
**区块 2（新）：发愿人数**
- 🖥️ 看到：`🙏 已有 N 人发愿`
- 数据来源：`GET /api/events/:id/vow-count`（COUNT UserPracticeVow WHERE context=event AND eventId）
- 静态展示，无需轮询
...
| 用户状态 | 展示 | 操作 |
|---|---|---|
| 法会任何状态，无愿 | `发法会愿` 按钮 | 触发发愿 Sheet |
| 法会未结束，有愿 | 我的发愿：N 遍 | `回向` |
| 法会已结束，有愿 | 我的发愿：N 遍 | `回向` |
| 法会已结束，无愿 | 「此法会已结束」 | 无操作 |

边缘情况：
- 有愿即可触发回向，不依赖任何提交记录（发愿量 = 承诺量，线下自行完成）
...
> **需求变更（2026-05-27）**：法会线下数量由师兄们自行上报，App 不再提供个人记数入口。
> `POST /api/events/:id/count` 端点废弃，不再写 `EventCount`，不再更新 `UserPracticeVow.currentCount`。
> 发愿人数通过 `GET /api/events/:id/vow-count` 展示，回向触发条件改为「有法会愿」（见 UI-16）。
...
**白话**：有法会愿即可点「回向」，纯 UI 仪式，不写 DB，可重复点击。

规则：
- 触发时机：法会进行中或已结束均可
- **触发条件（更新）**：该用户对该法会有 `UserPracticeVow`（发了愿即可，不依赖 EventCount 提交记录）
- 展示：发愿数量（目标量）+ 固定回向文字
...
注意：
- **需求变更（2026-05-27）**：法会不再写 `EventCount`，法会愿的 `currentCount` 不再实时更新，`SUM(EventCount.count)` 进度条逻辑废弃
- 法会愿状态仅靠定时任务按时间判断（进行中 → on_track，已结束 → completed），与数量无关
- 约修愿状态逻辑不变：`currentCount` 由每次打卡累加，到期标 completed
- `currentStatus` 仅供辅导员端愿管理列表筛选，学员端不展示

> 核对结论：✅ 正确（法会愿已移除数量追踪，仅按时间边界判断）
...
| 藏历日历「今天」显示 | `User.timezone` | 按用户本地时区确定公历日期后换算藏历；与法会边界分开，日历是个人视角 |
