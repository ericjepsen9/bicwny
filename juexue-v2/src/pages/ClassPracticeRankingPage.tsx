// ClassPracticeRankingPage · /class/:id/practice-ranking
//   班级修学（念诵 / 计数）排行 · 周 / 月 / 全部 切换
//   与 ClassMeditationRankingPage 同款样式 · 数据源换成 practiceDailySummary 聚合
...
import { type PracticeRankingPeriod, useClassPracticeRanking } from '@/lib/queries';

export default function ClassPracticeRankingPage() {
  const { s } = useLang();
  const { id } = useParams<{ id: string }>();
  const classId = id || '';
  const { user } = useAuth();
  const [period, setPeriod] = useState<PracticeRankingPeriod>('week');
  const ranking = useClassPracticeRanking(classId || null, period);
    ...
    <div style={{ padding: 'var(--sp-3)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', marginBottom: 'var(--sp-3)' }}>
        ...
        <h1 style={{ flex: 1, fontFamily: 'var(--font-serif)', fontWeight: 700, fontSize: '1.125rem', color: 'var(--ink)', letterSpacing: 1.5, margin: 0 }}>
          📿 {s('班级念诵排行', '班級念誦排行', 'Class chanting ranking')}
        </h1>
      </div>

      {/* 周期切换 */}
      <div style={{ display: 'flex', gap: 6, marginBottom: 'var(--sp-3)' }}>
        {(['week', 'month', 'all'] as PracticeRankingPeriod[]).map((p) => (
          <button
            key={p}
            type="button"
            onClick={() => setPeriod(p)}
      ...
      {ranking.isLoading ? (
        ...
        <div className="glass-card-thick" style={{ padding: 'var(--sp-7)', textAlign: 'center', color: 'var(--ink-3)' }}>
          ...
          {(ranking.error as ApiError | undefined)?.message ?? s('暂无念诵记录', '暫無念誦記錄', 'No chanting records yet')}
        </div>
      ) : (
