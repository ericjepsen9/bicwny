...
> 是否有跨时区班级（如北京班、纽约班同时运营）？

暂定：✅ 加字段但不强制填写，`city String?` + `timezone String?`，自学师兄用设备时区。
...
> ⚠️ 注意：DB_DIFF 文档原写「≥30min=1座，≥15min=0.5座」，测试场景文档写「45=1，20=0.5，10=0」，以测试场景文档为准。需用户最终确认阈值。🔲
...
| 编号 | 问题 | 影响 |
|---|---|---|
| Q1 | 班级时区（2C）：是否有跨时区班级？ | Class 表是否需要 city/timezone 字段 |
| Q2 | 座次阈值：测试文档写 45/20 分钟，DB_DIFF 写 30/15 分钟，以哪个为准？ | PracticeLog 座次计算逻辑 |
| Q3 | 主麦能否改师兄 auto 愿的每日目标量（节奏）？ | 6C 到期日变更权限表 |
