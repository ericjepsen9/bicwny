...
export async function chat(
  ...
  for (const { cfg, model } of candidates) {
    ...
    try {
      const resp = await getProvider(cfg.name).chat({
        ...
        requestId,
        timeoutMs: ctx.timeoutMs ?? LONG_TIMEOUT_SCENARIOS[scenario] ?? DEFAULT_LLM_TIMEOUT_MS,
      });
  ...
  // 加机器可读 code 让前端区分 LLM 故障 vs 网络错 展示不同 UI
  throw UpstreamError('AI 服务暂不可用，请稍后再试', {
