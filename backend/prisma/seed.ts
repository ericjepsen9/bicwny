...
//   1.3 multi + open 题                                  ✓
//   1.4 sort + match 题                                  ✓
//   1.5 LLM providers + scenario + prompt 模板           ← 当前
...
// ═══════════════════════════════════════════════════════════════
// 1.5  LLM 配置（providers + scenario + prompt template）
// ═══════════════════════════════════════════════════════════════
...
const SCENARIO_ID = 'scenario_open_grading';
const PROMPT_TEMPLATE_ID = 'prompt_open_grading_v3_1';
...
async function seedLlmProviders() {
  ...
  await prisma.llmProviderConfig.upsert({
    ...
    create: {
      ...
      role: 'primary',
      priority: 1,
      // 占位：100M tokens/年、约 8.3M/月、10k req/日、60 rpm、5 并发
  ...
  // 兜底：Claude Haiku，pay-as-you-go，月预算上限 200 USD
  await prisma.llmProviderConfig.upsert({
    where: { name: 'claude' },
    update: {},
    create: {
...
async function seedLlmPromptTemplate() {
  const content = `你是佛法题库的评分助手，负责评估学员对开放题的作答。
...
async function seedLlmScenario() {
...
async function main() {
  ...
  await seedLlmPromptTemplate();
  await seedLlmScenario();
  console.log('\n✅ Seed done — 20 questions + 2 LLM providers + 1 scenario + 1 prompt template');
}
